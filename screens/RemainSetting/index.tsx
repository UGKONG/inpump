...
export default function RemainSettingScreen() {
  const navigation = useNavigation();
  const [value, setValue] = useState<Value>({value: 250});
  ...
  return (
    <Container.Scroll>
      <SettingDescription text={description.remain} />

      <ItemGroup title="현재 잔량" style={{marginTop: 0}} />
      <Item title="250U" nonePress />
      <ItemGroup title="잔량 설정" style={{marginTop: 0}} />
      <Setting.Row>
        <Select
          unit="U"
          value={value?.value}
